<template>
  <section class="dept-grade-page">
    ...
    <div class="grade-card" v-if="deptGrades.length">
      ...
      <table class="grade-table">
        ...
        <tbody>
        <tr
          v-for="dept in filteredDeptGrades"
          :key="dept.departmentId"
          class="clickable-row"
          @click="openDetailModal(dept)"
        >
          ...
          <td class="actions">
            <button
              ...
              class="btn primary"
              @click.stop="openAssignModal(dept)"
            >
              등급 부여
            </button>
            ...
            <template v-else-if="!dept.gradeStatus || dept.gradeStatus === 'DRAFT'">
              ...
              <button
                class="btn danger"
                @click.stop="removeDeptGrade(dept.deptGradeId)"
              >
                삭제
              </button>
              ...
              <button
                class="btn primary"
                @click.stop="submitDeptGrade(dept.deptGradeId, dept.gradeReason)"
              >
                제출
              </button>
  ...
  <DeptGradeDetailModal
    v-if="showDetailModal"
    :dept="selectedDetail"
    @close="showDetailModal = false"
    @approved="loadDeptGrades"
  />
...
const submitDeptGrade = async (deptGradeId, gradeReason) => {
  if (!gradeReason || !gradeReason.trim()) {
    alert('제출 시에는 부여 사유를 입력해야 합니다')
    return
  }
