<template>
  <section class="cycle-page">
    <h2 class="page-title">평가 회차 관리 (관리자)</h2>
    ...
    <BaseCard>
      ...
      <div class="form">
        ...
        <div class="field">
          <label>회차명</label>
          <input v-model="form.cycleName" placeholder="예: 2026 하반기 평가" />
        </div>
    ...
    <BaseCard class="mt">
      ...
      <table class="table" v-if="filteredCycles.length">
        ...
        <tbody>
        <tr v-for="cycle in filteredCycles"
            :key="cycle.cycleId"
            class="row-click"
            @click="router.push(`/hr/cycles/${cycle.cycleId}`)">
